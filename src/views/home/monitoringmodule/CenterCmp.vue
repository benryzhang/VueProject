<template>
  <div class="center-cmp">
    ...
    <div class="l-dm">
      ...
      <nav class="nav">
        <ul>
          <li> <a href="#" @click="innerVisible = true"> <span> 平台简介 </span> </a> </li>
          <li> <a href="#" @click="outerVisible = true"> <span> 结构管理 </span> </a> </li>
        </ul>
      </nav>
      </div>
      <div class="r-dm">
  ...
  <!--弹框组件开始-----------------------start-->
    ...
    <el-dialog title="平台简介" customClass="customWidth" :visible.sync="innerVisible"  ref="structureset" :modal-append-to-body='false'>
    ...
    <el-dialog title="结构管理" :visible.sync="outerVisible"  width="50%" @close="addDialogClosed"  :modal-append-to-body='false'>
            <!-- 内容主体区域 -->
...
export default {
        ...
        methods: {
          ...
          handler ({BMap, map}) {
                ...
                // var point = new BMap.Point(111.625117, 33.409132);    
          ...
          async getStructInfo(BMap, map) {
            await this.$axios.get(this.api.getStructInfo,{params:{username:'admin',structure:this.GLOBAL.structure}}).then(response => {
                //const sensortype = JSON.parse(response)
                var objdata = JSON.parse(response);
								var structdata = objdata.alarm;
                this.structdata = structdata
                var structarray = [];
								if (structdata.length != 0) {
									for ( var i = 0; i < structdata.length; i++) {
                    var point = new BMap.Point(structdata[i].longid, structdata[i].latid);    
                    map.centerAndZoom(point, 12); 
										var mkr = new BMap.Marker(point);
										map.addOverlay(mkr);
										var label = new BMap.Label(structdata[i].structname, {offset : new BMap.Size(25, 8)}); // 创建文本标注对象
                    ...
                    mkr.setLabel(label);
										...
										var contentall = "<div style='margin-top:12px ' ><img src='images/picture.png'/ height='100' width='100%'></div>" + "<div><h1 style='font-size:14px;width:100%;text-align:center;margin:4px 0px'>" + structdata[i].structname + "</h1></div>" + "<div style='height:128px;overflow-y:auto'><p style='font-size:12px; color:#333;margin:0'>" + structdata[i].structinfo + "</p></div>" + "<div style='padding-top:2px'  onclick='blq(\"" + structdata[i].childurl + "\")'><a style='background: #42a4e0; border: 1px solid #1e7db9;color: #fff;display: block; float: right; font-size: 14px;line-height: 24px; text-align: center; width: 260px;cursor:pointer;'>" + structdata[i].childname + "</a></div>"
										this.addClickHandler(contentall, mkr, structdata[i].structid, structdata[i].structname);
                ...
                // //     //var icon = new BMap.Icon('images/target-gg.png', new BMap.Size(20, 32), {infoWindowAnchor : new BMap.Size(10, 0), infoWindowOffset : new BMap.Size(10, 0), anchor : new BMap.Size(10, 30) });
                    
                  
                // //     var point = new BMap.Point(structdata[i].longid, structdata[i].latid);
                // //     //map.centerAndZoom(point, 12);  
                // //     var marker = new BMap.Marker(point);
                // //     //var map = new BMap.Map("allmap", {enableMapClick : false, minZoom : 5, maxZoom : 17});
                // //     map.addOverlay(marker);
                // //     var label = new BMap.Label(structdata[i].structname, {offset : new BMap.Size(25, 8)}); // 创建文本标注对象
                // //     label.setStyle({background : "#10152f", color : "#fff", fontSize : "8px", height : "28px", lineHeight : "28px", border : "2px #00ffff  solid",padding:"0 6px"});
                ...
                // //       contentall = "<div style='margin-top:12px;' ><img src='"+structdata[i].picurl+"'/ height='100' width='100%'></div>" + "<div><h1 style='font-size:14px;width:100%;text-align:center;margin:4px 0px'>" + structdata[i].structname + "</h1></div>" + "<div style='height:128px;overflow-y:auto'><p style='font-size:12px; color:#333;margin:0'>" + structdata[i].structinfo + "</p></div>" + "<div style='bottom: 0px;position:absolute;right: 0;'  onclick='blq(\"" + structdata[i].structid + "\")'><a style='background: #42a4e0; border: 1px solid #1e7db9;color: #fff;display: block; float: right; font-size: 14px;line-height: 24px; text-align: center;cursor:pointer;'>" + structdata[i].structname + "</a></div>";
                // //     }else{
                ...
                // //     point = new BMap.Point(structdata[0].longid, structdata[0].latid);    
                // //     map.centerAndZoom(point, 12); 
          ...
          changeStruct(structid){
            ...
            for ( var i = 0; i < this.structdata.length; i++) {
                  ...
                  if (structid == this.structdata[i].structid) {
                        var point = new BMap.Point(this.structdata[i].longid, this.structdata[i].latid);
...
.el-dialog {
  ...
  .customWidth{
        ...
        max-height: 600px;
        overflow: auto;}
        .el-dialog__header{ border-bottom:1px solid #bcbcbc}
</style>